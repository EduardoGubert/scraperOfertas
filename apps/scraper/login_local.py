...
from apps.scraper.main import ScraperMLAfiliado
...
def export_cookies() -> bool:
    if not os.path.exists(BROWSER_DATA_DIR):
        ...
        print("       Faca login primeiro: python -m apps.scraper.login_local")
        return False
...
def show_status() -> None:
    ...
    if not status.get("exists"):
        ...
        print("    Execute: python -m apps.scraper.login_local")
        return
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        if cmd == "--status":
            ...
            print("Uso: python -m apps.scraper.login_local [--status|--export]")
    else:
